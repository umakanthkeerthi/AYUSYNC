...
class AyuSyncApp {
  ...
  private quickCards: NodeListOf<HTMLElement>;
  private seeAllPlanBtn: HTMLElement | null;
  private checkBtn1: HTMLButtonElement | null;
  private homeProgressRing: SVGCircleElement | null;
  private homeProgressPercent: HTMLElement | null;
  private userAvatarBtn: HTMLElement | null;
  private langSelectProfile: HTMLSelectElement | null;

  // Modals
  private notifModal: HTMLElement | null;
  private voiceModal: HTMLElement | null;
  private testDetailsModal: HTMLElement | null;
  private reportViewerModal: HTMLElement | null;
  private timeLockModal: HTMLElement | null;
  private claimDetailsModal: HTMLElement | null;
  private aiFactorsModal: HTMLElement | null;
  private mobileNavDrawer: HTMLElement | null;

  // Voice & Chat Elements
  private chatWindow: HTMLElement | null;
  private chatInput: HTMLInputElement | null;
  private sendMessageBtn: HTMLButtonElement | null;
  private openAyuSyncVoiceBtn: HTMLElement | null;
  private voiceMicHeaderBtn: HTMLButtonElement | null;
  private sendVoiceQueryBtn: HTMLButtonElement | null;
  private speechRecognizedText: HTMLElement | null;
  private voiceStatusText: HTMLElement | null;
  private activeVoiceCommand: string = 'open reports';
  ...
  constructor() {
    ...
    this.checkBtn1 = document.getElementById('checkBtn1') as HTMLButtonElement;
    this.homeProgressRing = document.getElementById('homeProgressRing') as unknown as SVGCircleElement;
    ...
    this.langSelectProfile = document.getElementById('langSelectProfile') as HTMLSelectElement;
    ...
    this.voiceMicHeaderBtn = document.getElementById('voiceMicHeaderBtn') as HTMLButtonElement;
    this.sendVoiceQueryBtn = document.getElementById('sendVoiceQueryBtn') as HTMLButtonElement;
  ...
  private setLanguage(lang: string): void {
    ...
    if (this.langSelectProfile) this.langSelectProfile.value = lang;
    ...
    document.querySelectorAll<HTMLElement>('[data-i18n]').forEach(el => {
      const key = el.dataset.i18n as keyof TranslationSet;
      if (key && dict[key]) {
  ...
  private setupTabRouter(): void {
    ...
    if (taskTest) taskTest.addEventListener('click', () => this.switchTab('tests'));
    if (taskAppt) taskAppt.addEventListener('click', () => this.switchTab('appointments'));
  }

  // ==========================================
  // DYNAMIC HOVER & TOUCH PAGE THEME
  // ==========================================
  private setupMedicationHoverEffect(): void {
    ...
    const activateMintTheme = (el?: Element | null) => {
      if (appContainer) appContainer.classList.add('mint-theme');
      if (mainContent) mainContent.classList.add('mint-theme');
      if (el) el.classList.add('active-theme-border');
    };

    const deactivateMintTheme = (el?: Element | null) => {
      if (appContainer) appContainer.classList.remove('mint-theme');
      if (mainContent) mainContent.classList.remove('mint-theme');
      if (el) el.classList.remove('active-theme-border');
    };
  ...
  // ==========================================
  public showTimeLockCenterAlert(medName: string, scheduledTimeDisplay: string, timeSymbol: string, timePeriodName: string, currentAppTimeStr: string): void {
    ...
    if (timeLockMedName) timeLockMedName.textContent = `${medName} ${timeSymbol}`;
    if (timeLockMedSchedule) timeLockMedSchedule.textContent = `Scheduled for ${scheduledTimeDisplay} ${timeSymbol} (${timePeriodName})`;
    ...
    if (timeLockCurrentTime) timeLockCurrentTime.textContent = currentAppTimeStr;
    if (this.timeLockModal) this.openModal(this.timeLockModal);
  }

  // ==========================================
  // POPDOWN ACCORDION & TIME LOGGING CONTROLLER
  // ==========================================
  private setupPopableMedicationsController(): void {
    ...
    const toggleHomeAccordion = () => {
      if (homeAccordion) {
        ...
        if (medicineCardArrowBtn) medicineCardArrowBtn.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
        if (taskMedsItem) {
          if (isOpen) taskMedsItem.classList.add('open-arrow');
          else taskMedsItem.classList.remove('open-arrow');
        }
      }
    };
    ...
    if (taskMedsItem) {
      taskMedsItem.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).closest('#checkBtn1')) return;
        toggleHomeAccordion();
      });
    }
    ...
    const bindCompactMedToggle = (
      ...
      timeSymbol: string = '🌅',
      timePeriodName: string = 'Morning'
    ) => {
      const btn = document.getElementById(btnId);
      const input = document.getElementById(timeInputId) as HTMLInputElement;
      const statusEl = document.getElementById(statusId);
      const row = document.getElementById(rowId);

      if (!btn || !input || !statusEl || !row) return;
      ...
      btn.addEventListener('click', (e) => {
        ...
        if (isLogged) {
          ...
          if (homeTimeAlert) homeTimeAlert.style.display = 'none';
          return;
        }
  ...
  private updateProgressRing(percent: number): void {
  ...
  // CHAT & VOICE ENGINE
  // ==========================================
  private appendChatMessage(text: string, sender: 'user' | 'ai'): void {
  ...
  private handleAiQuery(queryText: string): void {
  ...
  // ==========================================
  private setupAlexaVoiceController(): void {
    // Single tap on mic symbol does NOT open voice modal.
    document.querySelectorAll<HTMLButtonElement>('.voice-cmd-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        const cmd = chip.dataset.cmd || 'open reports';
        this.activeVoiceCommand = cmd;
        if (this.speechRecognizedText) this.speechRecognizedText.textContent = `"${cmd}"`;
      });
    });
    ...
    if (this.sendVoiceQueryBtn) {
      this.sendVoiceQueryBtn.addEventListener('click', () => {
        ...
        if (cmd.includes('report')) this.switchTab('reports');
        else if (cmd.includes('plan')) this.switchTab('plan');
        else if (cmd.includes('med')) this.switchTab('medications');
        else if (cmd.includes('appt') || cmd.includes('appointment')) this.switchTab('appointments');
        else if (cmd.includes('profile')) this.switchTab('profile');
        else if (cmd.includes('test')) this.switchTab('tests');
        else this.switchTab('home');
  ...
  private setupNotificationCenter(): void {
    ...
    const notifItems = document.querySelectorAll<HTMLElement>('.notif-card-item');
  ...
  private setupExtraTriggers(): void {
    ...
    if (reportItem2) {
      reportItem2.addEventListener('click', () => {
        ...
        if (title) title.textContent = "12-Lead ECG Resting Scan";
    ...
    document.querySelectorAll<HTMLElement>('.track-claim-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const claimId = btn.dataset.claim || 'CLM-90812';
        const title = document.getElementById('claimModalTitle');
        if (title) title.textContent = `Claim #${claimId} Live Tracking`;
    ...
    document.querySelectorAll<HTMLElement>('.compare-plan-btn').forEach(btn => {
      btn.addEventListener('click', () => {
    ...
    document.querySelectorAll<HTMLElement>('.view-plan-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const plan = btn.dataset.plan || 'Plan';
  ...
  private setupLiveLocationTracker(): void {
    ...
    const updateLiveLocationUI = (lat: number, lng: number, placeName?: string) => {
      currentLat = lat;
      currentLng = lng;
      const formattedCoords = `${lat.toFixed(4)}° N, ${lng.toFixed(4)}° E`;
      const timeNow = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });

      if (patientLiveAddressText) patientLiveAddressText.textContent = placeName || `Current Location Fix: ${formattedCoords}`;
      if (patientLiveCoords) patientLiveCoords.textContent = formattedCoords;
      if (patientLiveTimestamp) patientLiveTimestamp.textContent = `${timeNow} (Live Sync)`;
    };
    ...
    const startTracking = (mode: string) => {
      localStorage.setItem('ayusync_location_permission', mode);
      if (locationBadgeDot) locationBadgeDot.classList.remove('hidden');
    ...
    const stopTracking = () => {
      ...
      if (locationBadgeDot) locationBadgeDot.classList.add('hidden');
  ...
  private setupEmergencyTriggers(): void {
    ...
    let micResetTimer: any = null;

    const handleMicClick = (e: Event) => {
        ...
        if (payloadStr) {
          try {
            ...
            if (payload && payload.active) {
              ...
              if (sosModal) this.openModal(sosModal);
              return;
            }
          } catch(e) {}
        }
      ...
      if (pageMicBtn) {
        ...
        if (micClickCounter === 1) pageMicBtn.classList.add('mic-tap-1');
        else if (micClickCounter === 2) pageMicBtn.classList.add('mic-tap-2');
      }

      micResetTimer = setTimeout(() => { 
        micClickCounter = 0; 
        if (pageMicBtn) pageMicBtn.classList.remove('mic-tap-1', 'mic-tap-2');
      }, 1500);
    ...
    const pageMicBtn = document.getElementById('voiceMicBtnMain');
    if (pageMicBtn) pageMicBtn.addEventListener('click', handleMicClick);
    ...
    if (forceSendSosBtn) {
      forceSendSosBtn.addEventListener('click', () => {
        ...
        if (countdownModal) this.closeModal(countdownModal);
  ...
  private sosCountdownInterval: any = null;
  private sosCountdownSeconds = 5;
  private activeSosReason = '';

  private start5SecondSosCountdown(reasonText: string): void {
    ...
    if (subReason) subReason.textContent = `${reasonText} • Tap Cancel within 5 seconds if this was an accidental tap.`;
    if (secondsNum) secondsNum.textContent = '5';
    if (secondsText) secondsText.textContent = '5';

    if (countdownCircle) {
      countdownCircle.style.strokeDashoffset = '0';
    }

    if (countdownModal) this.openModal(countdownModal);
    ...
    this.sosCountdownInterval = setInterval(() => {
      ...
      if (secondsNum) secondsNum.textContent = `${this.sosCountdownSeconds}`;
      if (secondsText) secondsText.textContent = `${this.sosCountdownSeconds}`;
  ...
  private cancelSosCountdown(): void {
    if (this.sosCountdownInterval) {
      clearInterval(this.sosCountdownInterval);
      this.sosCountdownInterval = null;
    }
  ...
  private triggerEmergencySos(reasonText: string): void {
    // 1. Force switch to Homepage view
    this.switchTab('home');
    ...
    if (sosReason) sosReason.textContent = `${reasonText} • Transmitting live location fix to CareOS Ambulance Driver.`;

    const broadcastSosPayload = (addr: string, lat: number, lng: number) => {
      if (sosAddress) sosAddress.textContent = addr;
      const payload = {
        patientId: '10482',
