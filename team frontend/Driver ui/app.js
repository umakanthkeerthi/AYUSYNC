...
class CareOSDriverApp {
    constructor() {
        this.currentTab = 'trip';
        this.isArrived = false;
        this.isSirenOn = true;
        // REAL-TIME PATIENT EMERGENCY SOS LISTENER
        this.currentSosLat = 24.6180;
        this.currentSosLng = 73.9915;
    ...
    closeModal(modalEl) {
        if (modalEl)
            modalEl.classList.remove('open', 'drawer-open');
    }
    setupModals() {
        // Call Modal
        const triggerCall = () => this.openModal(this.callModal);
        if (this.emergencyCallBtn)
            this.emergencyCallBtn.addEventListener('click', triggerCall);
        if (this.drawerEmergencyBtn)
            this.drawerEmergencyBtn.addEventListener('click', triggerCall);
        if (this.cancelCallBtn)
            this.cancelCallBtn.addEventListener('click', () => this.closeModal(this.callModal));
        if (this.closeCallModalBtn)
            this.closeCallModalBtn.addEventListener('click', () => this.closeModal(this.callModal));
        // Nav Modal
        if (this.navigateBtn) {
        ...
        if (this.closeNavModalBtn)
            this.closeNavModalBtn.addEventListener('click', () => this.closeModal(this.navModal));
        if (this.closeNavModalBtn2)
            this.closeNavModalBtn2.addEventListener('click', () => this.closeModal(this.navModal));
    }
    setupActionButtons() {
        ...
        if (this.markArrivedBtn) {
            this.markArrivedBtn.addEventListener('click', () => {
                ...
                if (this.isArrived) {
                    ...
                    if (this.markArrivedBtn)
                        this.markArrivedBtn.textContent = '✓ Arrived at Apollo Hospital (Handover Complete)';
                    this.showToast('✅ Ambulance Arrived at ER Gate! Handover protocol initiated.');
                }
                else {
                    this.markArrivedBtn?.classList.remove('completed');
                    if (this.markArrivedBtn)
                        this.markArrivedBtn.textContent = 'Mark as Arrived';
        ...
        if (this.recenterMapBtn) {
            this.recenterMapBtn.addEventListener('click', () => {
                if (this.ambulanceSprite) {
                    ...
                    setTimeout(() => {
                        if (this.ambulanceSprite)
                            this.ambulanceSprite.style.transform = 'scale(1)';
                    }, 300);
                }
    ...
    setupSosDispatchListener() {
        ...
        const handleSosDispatch = (payload) => {
            if (!payload || !payload.active)
                return;
            if (payload.lat)
                this.currentSosLat = payload.lat;
            if (payload.lng)
                this.currentSosLng = payload.lng;
            if (driverPatientName)
                driverPatientName.textContent = `${payload.patientName} (ID #${payload.patientId})`;
            if (driverPickupAddress)
                driverPickupAddress.textContent = payload.pickupAddress || `GPS Fix (${payload.lat}, ${payload.lng})`;
            if (driverTimestamp)
                driverTimestamp.textContent = payload.timestamp || 'Just now';
            if (driverTriggerSource)
                driverTriggerSource.textContent = `Triggered via ${payload.reason || 'Patient Emergency SOS Signal'}`;
            if (driverSosModal) {
